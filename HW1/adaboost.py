...
class Adaboost:
    ...
    def selectBest(self, featureVals, iis, labels, features, weights):
        ...
        # row = np.size(x, 0)
        # column = np.size(x, 1)
        dataLen = np.size(featureVals, 1)   
        bestError = 1e8
        for j in range(len(features)):
            # for each feature j, train a classifier hj
            """
            x = np.array(featureVals[j])
            for element in x:
                if x < 0:
                    x = 1
                else:
                    x = 0
            """
            temp = WeakClassifier(features[j], 0, 1)
            error = 0
            for i in range(dataLen):
                # evaluate error for each feature
                error += weights[i] * abs(temp.classify(iis[i]) - labels[i])                                
            if error < bestError:
                bestError = error
                bestClf = temp
