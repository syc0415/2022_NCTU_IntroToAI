...
class Agent():
    ...
    def learn(self):
        ...
        b_state = torch.tensor(sample[0], dtype = torch.float)
        b_action = torch.tensor(sample[1], dtype = torch.long).unsqueeze(-1)
        b_reward = torch.tensor(sample[2], dtype = torch.float).unsqueeze(-1)
        b_next_state = torch.tensor(sample[3], dtype = torch.float)
        b_done = torch.tensor(sample[4], dtype = torch.float).unsqueeze(-1)
        
        q_eval = self.evaluate_net(b_state).gather(1, b_action)
        q_next = self.target_net(b_next_state).detach()
        q_max = q_next.max(1) # 行最大
        q_max = q_max.values.unsqueeze(-1)
        q_target = b_reward + self.gamma * q_max   
        ...
        '''
            try:
                pre = torch.load("./Tables/DQN.pt")
                init = torch.tensor(self.env.reset(), dtype = torch.float)
                target = self.target_net(init).detach()
                cur = target.max(0).values.unsqueeze(-1)[0]
                if cur > pre.max(0)[0]:                    
                    torch.save(self.target_net.state_dict(), "./Tables/DQN.pt")
            except:
                torch.save(self.target_net.state_dict(), "./Tables/DQN.pt")
    ...
    def choose_action(self, state):
        ...
        with torch.no_grad():
            ...
            if p < self.epsilon:
                ...
                action_val = self.evaluate_net(x)
    ...
    def check_max_Q(self):
        ...
        init = torch.tensor(self.env.reset(), dtype = torch.float)
        target = self.target_net(init).detach()
        max_q = target.max(0) # 列最大
        max_q = max_q.values.unsqueeze(-1)
        return max_q[0]
